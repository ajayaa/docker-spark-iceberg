...
def write_to_iceberg(batch_df, batch_id):
    if not batch_df.rdd.isEmpty():
        batch_df.writeTo("demo.my_db.audit_log").append()
...
def main():
    ...
    spark = SparkSession.builder.appName("auditLog").config(conf=conf).getOrCreate()
    ...
    # Create the table
    spark.sql("CREATE TABLE IF NOT EXISTS demo.my_db.audit_log (key STRING, value STRING) USING iceberg")
    # Insert data
    ...
    df = spark \
        ...
        .option("subscribe", "ripplingdb.aj") \
        .load()
    ...
    query = output_df.writeStream.foreachBatch(write_to_iceberg).outputMode("append").option("checkPointLocation", "/tmp/checkPoint2").start()
